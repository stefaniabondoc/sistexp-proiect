...
% TODO: De documentat cazurile cand nu are solutie
% TODO: Afisarea solutiilor in ordine descrescatoare a factorului de certitudine
% TODO: Afiseaza mesaj cand nu stun solutii
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
pornire :-
...
Not_FC is - FC, !.
realizare_scop(Scop, 100, _):-
fapt(Scop, _, av(_, nu_conteaza)).
...
transformare2(av(A,da),['yes',':',A]) :- !.
transformare2(not av(A,da), [no,':',A]) :- !.
...
% TODO: de adaugat optiuni nu stiu(neaga oricare depinde de atribut) si nu conteaza(wildcard)
% TODO: la intrebare booleana sea afiseaza de ex: te doare capul? [da, nu]

interogheaza(Atr,Mesaj,[da,nu],Istorie) :-
!,write(Mesaj),nl,write('( da nu )'),nl,nl,
...
de_la_utiliz(X,Istorie,TLista_opt) :-
append(TLista_opt, [nu_stiu, nu_conteaza], Lista_opt),
...
% TODO: De modificat 20 in valoare data la curs, probabil 40.
...
['#',i,':',Atr],lista_optiuni(M),afiseaza(Atr,P).
...
caracter_cuvant(C):-member(C,[44,59,58,63,33,46,41,40,35]).
